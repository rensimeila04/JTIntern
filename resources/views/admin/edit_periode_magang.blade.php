...
@section('content')
    <div class="p-6 space-y-6 bg-white rounded-lg">
        <h1 class="text-xl font-medium text-neutral-900">Edit Periode Magang</h1>
        <form id="editPeriodeForm" action="{{ route('admin.periode_magang.update', $periodeMagang->id_periode_magang) }}"
                ...
                <div class="space-y-4 w-full">
                    <div class="w-full">
                        ...
                        <input type="text" id="nama_periode" name="nama_periode"
                            ...
                            placeholder="Contoh: Periode Magang 2025 Semester Ganjil">
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="space-y-4 w-full">
                    <div class="w-full relative">
                        <label for="tanggal_mulai" class="block text-sm font-medium mb-2 dark:text-white">Tanggal
                            Mulai</label>
                        ...
                        <input type="hidden" name="tanggal_mulai" id="startDateHidden"
                            value="{{ old('tanggal_mulai', $periodeMagang->tanggal_mulai) }}">
            ...
            <div class="flex justify-end gap-3 w-full">
                <a href="{{ route('admin.periode_magang') }}" class="btn-secondary">
                    Batal
                </a>
    ...
    <div id="editConfirmModal"
        ...
        role="dialog" tabindex="-1" aria-labelledby="editConfirmModal-label">
        <div
            ...
            <div
                ...
                <div class="flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-neutral-700">
                    <h3 id="editConfirmModal-label" class="font-bold text-gray-800 dark:text-white">
                        Konfirmasi Edit Periode Magang
                    </h3>
                    <button type="button" id="closeEditConfirmModalBtn"
                ...
                <div class="p-4 overflow-y-auto">
                    <div class="text-center">
                        <div class="w-16 h-16 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            <x-lucide-calendar-range class="w-8 h-8 text-amber-600" />
                        </div>
                        ...
                        <div class="text-left space-y-2 bg-gray-50 p-4 rounded-lg">
                            ...
                            <div>
                                ...
                                <span id="editConfirmMulai" class="text-sm text-gray-900 ml-2"></span>
                            </div>
                            <div>
                                <span class="text-sm font-medium text-gray-600">Tanggal Selesai:</span>
                                <span id="editConfirmSelesai" class="text-sm text-gray-900 ml-2"></span>
                            </div>
                            <div>
                                <span class="text-sm font-medium text-gray-600">Durasi:</span>
                                <span id="editConfirmDurasi" class="text-sm text-gray-900 ml-2"></span>
                            </div>
                        </div>
                        <p class="mt-4 text-sm text-gray-600">
                            Apakah Anda yakin ingin menyimpan perubahan data periode magang ini?
                        </p>
                    </div>
                </div>
                <div
                    class="flex justify-center items-center gap-x-2 py-3 px-4 border-t border-gray-200 dark:border-neutral-700">
                    <button type="button" id="cancelEditConfirm"
                    ...
                    <button type="button" id="confirmEditSubmit"
                        class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-amber-600 text-white hover:bg-amber-700 focus:outline-hidden focus:bg-amber-700 disabled:opacity-50 disabled:pointer-events-none">
                        <span id="editConfirmButtonText">Ya, Perbarui</span>
                        <div id="editConfirmSpinner"
    ...
    <div id="editSuccessModal"
        class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none"
        role="dialog" tabindex="-1" aria-labelledby="editSuccessModal-label">
        <div
            class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
            <div
                class="flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm pointer-events-auto dark:bg-neutral-900 dark:border-neutral-800">
                <div class="flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-neutral-700">
                    ...
                    <button type="button" id="closeEditSuccessModalBtn"
                        class="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-hidden focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:hover:bg-neutral-600 dark:text-neutral-400 dark:focus:bg-neutral-600"
                        aria-label="Close">
                        <span class="sr-only">Close</span>
                        <x-lucide-x class="size-4" />
                ...
                <div class="p-4 overflow-y-auto">
                    <div class="text-center">
                        ...
                        <h4 class="text-lg font-semibold text-gray-900 mb-2">Data Periode Magang Berhasil Diperbarui</h4>
                        <p class="text-sm text-gray-600 mb-4">
                            Perubahan data periode <span id="editSuccessPeriodName"
                                class="font-semibold text-gray-900"></span> telah berhasil disimpan.
                        </p>
                ...
                <div
                    class="flex justify-center items-center gap-x-2 py-3 px-4 border-t border-gray-200 dark:border-neutral-700">
                    <button type="button" id="backToPeriodListBtn"
                        class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-800 dark:text-white dark:hover:bg-neutral-800">
                        <x-lucide-list class="w-4 h-4" />
                        Kembali ke Daftar
                    </button>
                    <button type="button" id="editAnotherBtn"
    ...
    <script>
        ...
        function validateForm() {
            ...
            for (const field of requiredFields) {
                ...
                if (!element.value.trim()) {
                    ...
                    if (field.id === 'startDateHidden') {
                        document.getElementById('startDateInput').focus();
                    } else if (field.id === 'endDateHidden') {
        ...
        function showEditConfirmModal() {
            ...
            document.getElementById('editConfirmNama').textContent = namaPeriode;
            document.getElementById('editConfirmMulai').textContent = tanggalMulai;
            document.getElementById('editConfirmSelesai').textContent = tanggalSelesai;
            document.getElementById('editConfirmDurasi').textContent = durasiText;
        ...
        function showEditSuccessModal(periodName) {
            document.getElementById('editSuccessPeriodName').textContent = periodName;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            @if(session('success'))
                const periodName = '{{ $periodeMagang->nama_periode }}';

                // Delay showing modal to ensure everything is loaded
                setTimeout(function () {
                    showEditSuccessModal(periodName);
                }, 500);
            @endif
            ...
            document.getElementById('confirmEditSubmit').addEventListener('click', function () {
                ...
                // Submit the form
                document.getElementById('editPeriodeForm').submit();
            });
            ...
            document.getElementById('backToPeriodListBtn').addEventListener('click', function () {
            ...
            // Close edit success modal when clicking outside
            document.getElementById('editSuccessModal').addEventListener('click', function (e) {
                const modalContent = this.querySelector('.bg-white');
                if (!modalContent.contains(e.target)) {
                    closeEditSuccessModal();
                }
