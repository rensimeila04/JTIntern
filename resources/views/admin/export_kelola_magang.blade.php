...
<head>
    ...
    <title>Data Magang JTIntern</title>
    <style>
        body {
...
<body>
    ...
    <h3 class="text-center">LAPORAN DATA MAGANG</h3>
    @if($filterStatus != 'all')
        <h4 class="text-center">Status: {{ ucfirst($filterStatus) }}</h4>
    @endif
    ...
    <table class="border-all" style="width: 60%;">
        ...
        <tbody>
            @foreach($statuses as $key => $status)
                <tr>
                    <td class="status-{{ $key }}">{{ $status['label'] }}</td>
    ...
    <div class="mt-4">
        <p><strong>Total Data Magang:</strong> {{ $totalMagang }} magang</p>
